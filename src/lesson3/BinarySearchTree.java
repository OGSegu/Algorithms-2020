...
public class BinarySearchTree<T extends Comparable<T>> extends AbstractSet<T> implements CheckableSortedSet<T> {
    ...
    private Node<T> removeRecursive(Node<T> begin, T element) {
        if (begin == null)
            return null;
        int comparison = element.compareTo(begin.value);
        if (comparison == 0) {
            ...
            begin = maximumValue(begin.left);
            begin.right = right;
            begin.left = removeRecursive(left, begin.value);
            return begin;
        } else if (comparison > 0) {
            begin.right = removeRecursive(begin.right, element);
        } else {
            begin.left = removeRecursive(begin.left, element);
        }
        return begin;
    }
